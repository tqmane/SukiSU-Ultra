...
{
    struct filename *filename, *ksud_filename;
    const char su[] = SU_PATH;
    int ret;
    ...
    if (strncmp(filename->name, su, sizeof(su)))
        return 0;

    pr_info("manual execve su found\n");
    ksud_filename = getname_kernel(KSUD_PATH);
    if (IS_ERR(ksud_filename))
        return PTR_ERR(ksud_filename);

    ret = escape_with_root_profile();
    if (ret) {
        putname(ksud_filename);
        return ret;
    }

    putname(filename);
    *filename_ptr = ksud_filename;
    return 0;
}
