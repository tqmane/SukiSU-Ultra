...
#ifdef CONFIG_KSU_SUSFS
static int do_susfs_cmd(void __user *arg, unsigned int cmd)
{
    void __user *argp = arg;

    ksu_handle_susfs_reboot_cmd(cmd, &argp);
    return 0;
}

#define DEFINE_SUSFS_HANDLER(_name, _cmd) \
    static int _name(void __user *arg) { return do_susfs_cmd(arg, _cmd); }

DEFINE_SUSFS_HANDLER(do_susfs_add_sus_path, CMD_SUSFS_ADD_SUS_PATH)
DEFINE_SUSFS_HANDLER(do_susfs_set_android_data_root_path, CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH)
DEFINE_SUSFS_HANDLER(do_susfs_set_sdcard_root_path, CMD_SUSFS_SET_SDCARD_ROOT_PATH)
DEFINE_SUSFS_HANDLER(do_susfs_add_sus_path_loop, CMD_SUSFS_ADD_SUS_PATH_LOOP)
DEFINE_SUSFS_HANDLER(do_susfs_add_sus_mount, CMD_SUSFS_ADD_SUS_MOUNT)
DEFINE_SUSFS_HANDLER(do_susfs_hide_sus_mnts_for_non_su_procs, CMD_SUSFS_HIDE_SUS_MNTS_FOR_NON_SU_PROCS)
DEFINE_SUSFS_HANDLER(do_susfs_add_sus_kstat, CMD_SUSFS_ADD_SUS_KSTAT)
DEFINE_SUSFS_HANDLER(do_susfs_update_sus_kstat, CMD_SUSFS_UPDATE_SUS_KSTAT)
DEFINE_SUSFS_HANDLER(do_susfs_add_sus_kstat_statically, CMD_SUSFS_ADD_SUS_KSTAT_STATICALLY)
DEFINE_SUSFS_HANDLER(do_susfs_add_try_umount, CMD_SUSFS_ADD_TRY_UMOUNT)
DEFINE_SUSFS_HANDLER(do_susfs_set_uname, CMD_SUSFS_SET_UNAME)
DEFINE_SUSFS_HANDLER(do_susfs_enable_log, CMD_SUSFS_ENABLE_LOG)
DEFINE_SUSFS_HANDLER(do_susfs_set_cmdline_or_bootconfig, CMD_SUSFS_SET_CMDLINE_OR_BOOTCONFIG)
DEFINE_SUSFS_HANDLER(do_susfs_add_open_redirect, CMD_SUSFS_ADD_OPEN_REDIRECT)
DEFINE_SUSFS_HANDLER(do_susfs_enable_avc_log_spoofing, CMD_SUSFS_ENABLE_AVC_LOG_SPOOFING)
DEFINE_SUSFS_HANDLER(do_susfs_add_sus_map, CMD_SUSFS_ADD_SUS_MAP)
DEFINE_SUSFS_HANDLER(do_susfs_show_version, CMD_SUSFS_SHOW_VERSION)
DEFINE_SUSFS_HANDLER(do_susfs_show_enabled_features, CMD_SUSFS_SHOW_ENABLED_FEATURES)
DEFINE_SUSFS_HANDLER(do_susfs_show_variant, CMD_SUSFS_SHOW_VARIANT)
DEFINE_SUSFS_HANDLER(do_susfs_sus_su, CMD_SUSFS_SUS_SU)
...
#ifdef CONFIG_KSU_SUSFS
    { .cmd = CMD_SUSFS_ADD_SUS_PATH, .name = "SUSFS_ADD_SUS_PATH", .handler = do_susfs_add_sus_path, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH, .name = "SUSFS_SET_ANDROID_DATA_ROOT_PATH", .handler = do_susfs_set_android_data_root_path, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SET_SDCARD_ROOT_PATH, .name = "SUSFS_SET_SDCARD_ROOT_PATH", .handler = do_susfs_set_sdcard_root_path, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_SUS_PATH_LOOP, .name = "SUSFS_ADD_SUS_PATH_LOOP", .handler = do_susfs_add_sus_path_loop, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_SUS_MOUNT, .name = "SUSFS_ADD_SUS_MOUNT", .handler = do_susfs_add_sus_mount, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_HIDE_SUS_MNTS_FOR_NON_SU_PROCS, .name = "SUSFS_HIDE_SUS_MNTS_FOR_NON_SU_PROCS", .handler = do_susfs_hide_sus_mnts_for_non_su_procs, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_SUS_KSTAT, .name = "SUSFS_ADD_SUS_KSTAT", .handler = do_susfs_add_sus_kstat, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_UPDATE_SUS_KSTAT, .name = "SUSFS_UPDATE_SUS_KSTAT", .handler = do_susfs_update_sus_kstat, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_SUS_KSTAT_STATICALLY, .name = "SUSFS_ADD_SUS_KSTAT_STATICALLY", .handler = do_susfs_add_sus_kstat_statically, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_TRY_UMOUNT, .name = "SUSFS_ADD_TRY_UMOUNT", .handler = do_susfs_add_try_umount, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SET_UNAME, .name = "SUSFS_SET_UNAME", .handler = do_susfs_set_uname, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ENABLE_LOG, .name = "SUSFS_ENABLE_LOG", .handler = do_susfs_enable_log, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SET_CMDLINE_OR_BOOTCONFIG, .name = "SUSFS_SET_CMDLINE_OR_BOOTCONFIG", .handler = do_susfs_set_cmdline_or_bootconfig, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_OPEN_REDIRECT, .name = "SUSFS_ADD_OPEN_REDIRECT", .handler = do_susfs_add_open_redirect, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ENABLE_AVC_LOG_SPOOFING, .name = "SUSFS_ENABLE_AVC_LOG_SPOOFING", .handler = do_susfs_enable_avc_log_spoofing, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_ADD_SUS_MAP, .name = "SUSFS_ADD_SUS_MAP", .handler = do_susfs_add_sus_map, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SHOW_VERSION, .name = "SUSFS_SHOW_VERSION", .handler = do_susfs_show_version, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SHOW_ENABLED_FEATURES, .name = "SUSFS_SHOW_ENABLED_FEATURES", .handler = do_susfs_show_enabled_features, .perm_check = always_allow },
    { .cmd = CMD_SUSFS_SHOW_VARIANT, .name = "SUSFS_SHOW_VARIANT", .handler = do_susfs_show_variant, .perm_check = always_allow },
#ifdef CONFIG_KSU_SUSFS_SUS_SU
    { .cmd = CMD_SUSFS_SUS_SU, .name = "SUSFS_SUS_SU", .handler = do_susfs_sus_su, .perm_check = always_allow },
#endif
#endif
    {
        .cmd = 0,
