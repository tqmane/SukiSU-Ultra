...
#else
struct selinux_ss *backup_sepolicy;
#endif
...
static void apply_kernelsu_policy_rules(struct policydb *db)
{
...
void apply_kernelsu_rules()
{
    ...
    db = &pol->policydb;
    apply_kernelsu_policy_rules(db);
...
out_unlock:
    ...
    if (!backup_sepolicy) {
        backup_sepolicy = ksu_dup_selinux_ss(selinux_state.ss);
        if (IS_ERR(backup_sepolicy)) {
            pr_err("failed to create backup selinux_ss: %ld\n", PTR_ERR(backup_sepolicy));
            backup_sepolicy = NULL;
        }
    }
    db = get_policydb();
